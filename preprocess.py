...
def create(image_list, data_list, save_folder):
    ...
    for i, img in tqdm(enumerate(image_list), desc="Embedding images", leave=False):
        timer += 1
        try:
            img_embed, seg_map = _embed_clip_sam_tiles(img.unsqueeze(0), sam_encoder) # generate default/s/m/l from single image, then obtain segmentation mask from different level
        except:
            raise ValueError(timer)

        lengths = [len(v) for k, v in img_embed.items()] # how many semantic embeddings from different level
        total_length = sum(lengths)
        total_lengths.append(total_length)
        
        if total_length > img_embeds.shape[1]: # if total_segment lenght > default length, we need to expand the img_embeds
