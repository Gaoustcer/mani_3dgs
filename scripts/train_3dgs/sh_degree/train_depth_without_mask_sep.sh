...
do
    if [[ $file == scene_0001* ]];then
        ...
        for ratio in ${ratios[*]};do
            ((port=$port+1000))
            ((port=$port%30000))
            CUDA_VISIBLE_DEVICES=$DEVICEID python train_depth.py -s ./real_data/$file -m logs/shdegree0/depth_nomask/$file_$ratio --port $port --pcd-path point_cloud_$ratio.pcd --test_iterations 1 1000 7000 12000 30000 --separate-cameras --sh_degree 0 &
            ((DEVICEID=$DEVICEID+1))
            ((DEVICEID=$DEVICEID%$DEVICENUM))
        done
        # echo $file
