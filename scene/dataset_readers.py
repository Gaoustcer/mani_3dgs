...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics): # iterate every key in the dict
        ...
        uid = intr.id
        R = np.transpose(qvec2rotmat(extr.qvec)) # Rotate matrix [4,4]
        # suppose qvec = (a,b,c,d)
        # The R matrix is
        # 1-2c^2-2d^2 2bc-2ad 2ac+2bd
        # 2bc+2ad 1-2b^2-2^d 2cd-2ab
        # 2bd-2ac 2ab+2cd 1-2b^2-2c^2
        # is the rotate version of above matrix
        T = np.array(extr.tvec) # transformer vector [3]
        # cam_intrinsics.params [f_x,f_y]
...
def readColmapSceneInfo(path, images, eval, llffhold=8): # path:root_path images "images"
    try:
        cameras_extrinsic_file = os.path.join(path, "sparse/0", "images.bin")
        cameras_intrinsic_file = os.path.join(path, "sparse/0", "cameras.bin")
        cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file) 
        # jeys with store each image
        # key [1,2,...,194]
        # value is imgs has qvec tvec
        # img.xys [N,2]
        # img.point3D_ids [N]
        # id/camera_id/name meta data
        cam_intrinsics = read_intrinsics_binary(cameras_intrinsic_file)
        # cam_int keys [1]
        # value odict_keys(['id', 'model', 'width', 'height', 'params'])
    except:
    ...
    reading_dir = "images" if images == None else images # images is the subdir name which store images
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir))
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)
    # cam_infos store camera list
    if eval:
    ...
    nerf_normalization = getNerfppNorm(train_cam_infos) # {'translate':array[3],'radius':num}

    ply_path = os.path.join(path, "sparse/0/points3D.ply")
    bin_path = os.path.join(path, "sparse/0/points3D.bin")
    txt_path = os.path.join(path, "sparse/0/points3D.txt")
    if not os.path.exists(ply_path): # read point cloud
    ...
    try:
        ...
        # pcd store color  points and normal dierection as the point cloud information [N,3] color are normalized colors is noramlize to (0,1) normals are np.zeros points is the coordinates
    except:
        pcd = None
